...
def get_input():
    print("Enter TAC lines (empty line to stop):")
    while True:
        line = input().strip()
        if line == "":
            break
...
def parse_line(line):
    # returns dict with parts of the instruction
    d = {}
    d['raw'] = line
    d['active'] = True
...
def instr_to_str(d):
    if not d['active']:
        return None
    if d['type'] == 'jump' or d['type'] == 'label' or d['type'] == 'other':
        return d['raw']
    if d['type'] == 'copy':
...
def constant_propagation(instrs):
    ...
    for d in instrs:
        if not d['active']:
            continue
        if d['type'] == 'jump' or d['type'] == 'label':
            continue
...
# predefined samples
samples = [
    ["t1 = 3 * 4", "t2 = t1 + 2", "x = t2", "t3 = 10 + 5"],
    ["x = 5", "y = 10", "t1 = x + y", "t2 = t1 * 2", "z = t2"],
    ["t1 = a + b", "t2 = t1", "t3 = t2 * c", "x = t3"],
    ["t1 = b * c", "t2 = a + t1", "t3 = b * c", "t4 = d + t3", "x = t2", "y = t4"],
    ["t1 = a * 1", "t2 = b * 2", "t3 = c + 0", "t4 = d * 0", "x = t1", "y = t2", "z = t3", "w = t4"],
    ["t1 = 4 * 2", "t2 = a + t1", "t3 = 4 * 2", "t4 = b + t3", "t5 = t2 * 1", "x = t5", "t6 = c + 0", "y = t6", "t7 = 10 + 5"],
]
...
print("1. Run sample inputs")
print("2. Enter your own TAC")
ch = input("Choice: ").strip()

if ch == '1':
    for i in range(len(samples)):
        print("\n========================================")
        print("Example: " + sample_names[i])
        instrs = [parse_line(l) for l in samples[i]]
        print_code(instrs, "Before Optimization:")
        instrs2 = [parse_line(l) for l in samples[i]]
    ...
    get_input()
    if tac:
        instrs = [parse_line(l) for l in tac]
        print_code(instrs, "Before Optimization:")
        instrs2 = [parse_line(l) for l in tac]
        instrs2, applied = run_optimization(instrs2)
        print_code(instrs2, "After Optimization:")
        print("Techniques applied: " + (", ".join(applied) if applied else "None"))
    else:
